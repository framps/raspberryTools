...
function isSupportedSystem() {
   ...
   local version
   version=$(grep -E "^VERSION_ID" $MOUNTPOINT_ROOT/etc/os-release | cut -f 2 -d "=" | sed 's/"//g')
...
function parseBLKID { # device uuid/partuuid/label
   local blkid
...
if ((UID != 0)); then
   error "Please invoke $MYSELF as root or with sudo"
fi
